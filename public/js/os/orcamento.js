$(function(){
	...
	Adicionarpecas = function(){
		...
		$.post(base_url+"peca/getpecacodigobarras",{codigobarra:varcodigobarra},function(e){
			...
			if(Dadospeca.estoque_atual < 1) {
				alert('O peca '+Dadospeca.nome+' está com estoque 0 neste momento, mas irei liberar para fazer orçamento, não se esqueça, caso cliente aprovar o orçamento será descontando -1 no estoque da peça, comprar a peça para repor no estoque que estará negativo.');
				semEstoque = true;
			}
			...
			if(semEstoque == false){
				...
				tr_peca += "<tr data-idpeca='"+Dadospeca.id_peca+"' >";	
					...
					tr_peca += "<td>"+Dadospeca.modelo+"</td>";
					tr_peca += "<td>"+Dadospeca.marca+"</td>";
					tr_peca += "<td><select onchange='calcularTotal();'>";
					for(var j=1;j < 10;j++){
						tr_peca += "<option value="+j+">"+j+" quantidade fantasia sem estoque.</option>";
					}
					tr_peca += "</select></td>";
					tr_peca += "<td class='valor_venda'>"+Dadospeca.preco_venda+"</td>";
					tr_peca += "<td><i class='fa fa-times' onclick='removerpeca("+Dadospeca.id_peca+");' data-idpeca='"+Dadospeca.id_peca+"' style='color: red; cursor: pointer;'></i></td>";
