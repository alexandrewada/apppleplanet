$(function(){
  	...
  	getPrecoTroca = function() {
  		var produto 			    = $("select[name='id_produto_troca'] option:selected").data('json');
  		return produto.preco_troca;
  	}


  	$("select[name='id_produto_troca'],input[name='preco_compra']").change(function(){
  		var valor_troca 			= getPrecoTroca();
  		var valor_negociado			= $("input[name='preco_compra']").val();
  		var valor_diferencia  		= (valor_troca-valor_negociado).toFixed(2);
      	$("#total_produto").html(valor_diferencia);
  	});


    $("select[name='id_produto_troca']").trigger('change');
	...
	$("input[name='notafiscal']").change(function() {
        if($(this).is(":checked")) {
           ...
           $("input[name='cnpj_cliente']").show("slow");
        } else {
           $("input[name='cnpj_cliente']").hide();   
